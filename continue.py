...
last_save = int(np.floor(Tlast/writeInterval_plots))*writeInterval_plots
Tstart = last_save+1
header = ['Time', 'NoBlobs', 'Evolution_time', 'Circulation']
with open('{}/times_continued_{}.csv'.format(data_dir,case), 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    # write the header
    writer.writerow(header)
    writer.writerows(np.c_[times_data['Time'][:last_save],times_data['NoBlobs'][:last_save],times_data['Evolution_time'][:last_save],times_data['Circulation'][:last_save]])

blobs_file = os.path.join(data_dir,'blobs_{}_{n:06d}.csv'.format(case,n=last_save))
...
blobs = pHyFlow.blobs.Blobs(wField,vInf,nu,deltaTc,blobs_sigma,overlap,xShift,yShift,
...
for timeStep in range(Tstart, nTimeSteps+1):
    ...
    with open('{}/times_continued_{}.csv'.format(data_dir,case), 'a', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
...
if plot_flag == True:
    ...
    t_norm = np.array([])
    #Line plots
    times_file = os.path.join(data_dir,"times_continued_{}.csv".format(case))
    times_data = pandas.read_csv(times_file)
